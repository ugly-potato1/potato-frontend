...
export default function Buying() {
    ...
    useEffect(() => {
      ...
      const fakeProductData = [
        { id: 1, number: '선택 1', details: '#멋쟁이 토마토', name : '토마토',weight : '10kg', price: '10,000',quantity : 0, imageUrl: Tomato },
        { id: 2, number: '선택 2', details: '#멋쟁이 매실', name : '매실',weight : '10kg', price: '20,000', quantity : 0, imageUrl: Plum },
        { id: 3, number: '선택 3', details: '#멋쟁이 자두', name : '자두',weight : '10kg', price: '30,000', quantity : 0, imageUrl: Peach },
        { id: 3, number: '선택 3', details: '#멋쟁이 자두', name : '자두',weight : '10kg', price: '30,000', quantity : 0, imageUrl: Peach },
        { id: 3, number: '선택 3', details: '#멋쟁이 자두', name : '자두',weight : '10kg', price: '30,000', quantity : 0, imageUrl: Peach },
        { id: 3, number: '선택 3', details: '#멋쟁이 자두', name : '자두',weight : '10kg', price: '30,000', quantity : 0, imageUrl: Peach },
        // 추가가능
      ...
      <HeadLine></HeadLine>
        <ContentBox>
          ...
          <DescriptionContainer>
            <Info>마을의 펀딩 &#62;<a>청주</a></Info>
            <Title>신품종 킹머스켓 & 샤인머스켓 <br/>최초공개 추석 한정판</Title>
            <Description>
            농부의 마음으로 정직하고 신선하게프리미엄 & 신상 과일을큐레이션 하여 선물드립니다
            </Description>
            <ButtonBox>
                <ShareButton>
  ...
  <ProductList>
    {products.map((product) => (
      <Product key={product.id}>
        <img src={product.imageUrl} alt={`상품 이미지 - ${product.name}`} />
        <OptionContainer>
          <ProductInfo>
            {product.number} <a>{product.details}</a>
          </ProductInfo>
          <ProductName>품 명 <b>{product.name}</b></ProductName>
          <ProductWeight>무 게 <b>{product.weight}</b></ProductWeight>
          <ProductPrice>가 격<b>{product.price}원</b></ProductPrice>
          <QuantityControl>
            <QuantityButton onClick={() => handleQuantityChange(product, setProducts)(-1)}>-</QuantityButton>
            <ProductQuantity>{product.quantity}</ProductQuantity>
            <QuantityButton onClick={() => handleQuantityChange(product, setProducts)(1)}>+</QuantityButton>
          </QuantityControl>
          <SelectButton onClick={() => handleSelectClick(product)}>담기</SelectButton>
        </OptionContainer>
      </Product>
    ))}
  </ProductList>
...
const Product = styled.div`
  ...
  background: #FFF;
  border-radius : 10px;
  filter: drop-shadow(0px 1px 15px rgba(0, 0, 0, 0.20));
  img {
    width: 100%;
    height: 50%;
    object-fit: cover;
